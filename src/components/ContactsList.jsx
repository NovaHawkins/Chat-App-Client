...
const ContactsList = ({ user, setOpenChat }) => {
  ...
  const handleOpenChat = async (chatType, contact, room) => {
    if (chatType == "private") {
      ...
      await axios.get(`${baseURL}/users/chat/${room._id}`).then(async (res) => {
        setOpenChat({ chatId: res.data.chat._id, chatType: res.data.chat.type, members: res.data.chat.members, title: res.data.chat.name })
      })
    }
  }
  ...
  return (
    <div id='contactsList'>
      <button onClick={() => setShowContacts(!showContacts)}>
        Users
        {showContacts ? <i className='bi bi-chevron-up' /> : <i className='bi bi-chevron-down' />}
      </button>
      ...
      <button onClick={() => setShowRooms(!showRooms)}>
        Rooms
        {showRooms ? <i className='bi bi-chevron-up'/> : <i className='bi bi-chevron-down'/>}
      </button>
      <ul className='rooms'>
